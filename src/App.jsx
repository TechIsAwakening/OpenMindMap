...
const VIEWBOX = { minX: -640, minY: -380, width: 1280, height: 760 }

const INITIAL_NODES = [
  { id: 'root', label: 'Sujet principal', parentId: null, positionOverride: null },
  { id: 'node-1', label: 'Idée clé #1', parentId: 'root', positionOverride: null },
  { id: 'node-2', label: 'Idée clé #2', parentId: 'root', positionOverride: null },
]
...
function computeLayout(nodes) {
  ...
  nodes.forEach((node) => {
    if (!node.positionOverride) return
    ...
    positions[node.id] = {
      ...current,
      x: node.positionOverride.x,
      y: node.positionOverride.y,
      isOverride: true,
    }
  })
...
function App() {
  ...
  const resetNodePosition = (nodeId) => {
    setNodes((prev) =>
      prev.map((node) =>
        node.id === nodeId
          ? {
        ...
        <section className="canvas-wrapper">
          ...
          >
            ...
            {nodes.map((node) => {
              ...
              const hasManualPosition = Boolean(node.positionOverride)

              return (
                <g
                  key={node.id}
                  transform={`translate(${nodePos.x}, ${nodePos.y})`}
                  className={`mindmap-node ${isSelected ? 'is-selected' : ''} ${isDragging ? 'is-dragging' : ''}`}
                  onClick={(event) => {
                  ...
                  onPointerDown={(event) => {
                    ...
                    const currentPosition = positions[node.id]
                    if (!svgElement || !currentPosition) return
                  ...
                  onPointerMove={(event) => {
                    ...
                    const deltaX = (event.clientX - dragState.startPointer.x) * dragState.scaleX
                    const deltaY = (event.clientY - dragState.startPointer.y) * dragState.scaleY

                    const nextX = dragState.startPosition.x + deltaX
                    const nextY = dragState.startPosition.y + deltaY
                ...
                >
                  ...
                  >
                    <div
                      className={`mindmap-node-card ${isSelected ? 'is-selected' : ''} ${
                        isRoot ? 'is-root' : ''
                      } ${isDragging ? 'is-dragging' : ''} ${hasManualPosition ? 'has-manual-position' : ''}`}
                      role="button"
                      tabIndex={0}
                    >
                      {isSelected && hasManualPosition ? (
                        <div className="mindmap-node-toolbar">
                          <button
                            type="button"
                            className="node-toolbar-badge"
                            onPointerDown={(event) => {
                              event.stopPropagation()
                            }}
                            onClick={(event) => {
